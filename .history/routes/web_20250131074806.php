...
use App\Http\Controllers\Admin_pembayaranController;
use Illuminate\Http\Request;
use App\Http\Controllers\AdminPembayaranController;

Route::get('/admin/pembayaran', [AdminPembayaranController::class, 'index'])->name('admin.pembayaran');
Route::patch('/admin/pembayaran/confirm/{id_pemesanan}', [AdminPembayaranController::class, 'confirm'])->name('admin.pembayaran.confirm');

Route::get('/admin/pembayaran', [AdminPembayaranController::class, 'index'])->name('admin.pembayaran');

Route::get('/bayar/{id_pemesanan}', [BayarController::class, 'index'])->name('bayar.index');
Route::post('/bayar/confirm/{id_pemesanan}', [BayarController::class, 'confirm'])->name('bayar.confirm');

Route::get('/pemesanan/cancel-timeout/{id}', [PemesananController::class, 'cancelTimeout'])
    ->name('pemesanan.cancel_after_timeout');

Route::get('/admin_pembayaran',[Admin_pembayaranController::class,'index'])->name('admin_pembayaran.index');


Route::get('/get-harga/{id_kelas}', function ($id_kelas) {
...
Route::patch('/pemesanan/{id}/confirm-payment', [PemesananController::class, 'confirmPayment'])->name('pemesanan.confirmPayment');


Route::get('/bayar/{id_pemesanan}', [BayarController::class, 'index'])->name('bayar.index');
Route::post('/bayar/confirm/{id_pemesanan}', [BayarController::class, 'confirm'])->name('bayar.confirm');
Route::get('/pemesanan/{id}/cancel', [PemesananController::class, 'cancel'])->name('pemesanan.cancel.get');
Route::delete('/pemesanan/delete-cancelled', [PemesananController::class, 'deleteCancelled'])->name('pemesanan.deleteCancelled');

// Routes for BusKelas
Route::prefix('bus-kelas')->group(function () {
    // Display a listing of BusKelas
    Route::get('/', [BusKelasController::class, 'index'])->name('bus-kelas.index');

    // Show the form for creating a new BusKelas
    Route::get('/create', [BusKelasController::class, 'create'])->name('bus-kelas.create');

    // Store a newly created BusKelas
    Route::post('/store', [BusKelasController::class, 'store'])->name('bus-kelas.store');

    // Delete a specific BusKelas
    Route::delete('/{id_bus}/{id_kelas}', [BusKelasController::class, 'destroy'])->name('bus-kelas.destroy');
});
...
Route::get('detail_pemesanan/{id}/edit', [DetailPemesananController::class, 'edit'])->name('detail_pemesanan.edit');
Route::put('detail_pemesanan/{id}', [DetailPemesananController::class, 'update'])->name('detail_pemesanan.update');
...
Route::delete('detail_pemesanan/destroy/{id}', [DetailPemesananController::class, 'destroy'])->name('detail_pemesanan.destroy');
...
Route::prefix('bus')->middleware(['auth', 'adminOnly'])->group(function () {
