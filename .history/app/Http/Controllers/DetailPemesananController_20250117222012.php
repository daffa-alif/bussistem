    ...
    public function update(Request $request, $id)
{
    ...
    // Fetch the KursiBus with related Kelas and Harga using eager loading
    $kursiBus = KursiBus::with('kelas.harga')->findOrFail($validated['id_kursi_bus']);

    // Ensure the KursiBus has a Kelas and a valid Harga
    if (!$kursiBus->kelas || !$kursiBus->kelas->harga) {
        return redirect()->back()->withErrors(['error' => 'Harga tidak ditemukan untuk kursi ini.']);
    }

    // Filter the correct Harga if needed (for example, by selecting the most recent or specific condition)
    $harga = $kursiBus->kelas->harga; // This is the price you are fetching

    // Ensure that you're getting the right price based on business rules, if there are multiple.
    // Example: Select the latest or specific condition
    $harga = Harga::where('id_kelas', $kursiBus->kelas->id_kelas)
                  ->latest()  // You can use other conditions if needed (like specific dates or type)
                  ->first();
    ...
    $detailPemesanan->update([
        ...
        'harga_kursi' => $harga->harga,  // Correctly fetch the price
        'total_harga' => $harga->harga,  // Calculate the total price if needed
