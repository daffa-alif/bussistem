...
{
    // Fetch all users (to display in the user selection dropdown)
    $users = User::all();
    ...
    // Fetch available kursi_bus data
    $kursi_bus = KursiBus::with(['bus', 'kelas'])->where('status_kursi', 'available')->get();

    // Pass the users, jadwal, and kursi_bus to the view
    return view('pemesanan.create', compact('jadwal', 'kursi_bus', 'users'));
}
...
public function store(Request $request)
{
    ...
    $validated = $request->validate([
        ...
        'id_pengguna' => 'nullable|exists:pengguna,id_pengguna', // Allow for manual user input, if blank, use authenticated user
    ]);
