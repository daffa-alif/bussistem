...
class DetailPemesananController extends Controller
{
    ...
    public function create($id_pemesanan)
    {
        ...
        // Retrieve the available kursi
        $kursiBus = KursiBus::where('status_kursi', 'available')->get();
    
        // Get total_harga from the session (if passed before)
        $totalHarga = session('total_harga');
    
        // You can also pass an empty or default value for $id_kursi
        $id_kursi = $request->input('id_kursi', null); // Defaulting to null if not found in request
    
        return view('detail_pemesanan.create', [
            'id_pemesanan' => $id_pemesanan,  // Pass the id_pemesanan
            'kursiBus' => $kursiBus,  // Pass the kursi available
            'total_harga' => $totalHarga,  // Pass the total_harga
            'id_kursi' => $id_kursi,  // Pass id_kursi (may be null initially)
        ]);
    }
