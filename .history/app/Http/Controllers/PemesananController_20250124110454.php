    ...
    public function index()
{
    ...
    // Add remaining time calculation for each booking
    foreach ($pemesanan as $item) {
        $created_at = $item->created_at; // Timestamp when the booking was created
        $current_time = now(); // Get the current time

        // Calculate the difference in minutes between the current time and created_at
        $time_diff = $current_time->diffInMinutes($created_at);

        // Remaining time for the booking to be confirmed (15 minutes)
        $remaining_time = 15 - $time_diff;

        // Ensure remaining time is not negative
        $item->remaining_time = max(0, $remaining_time);
    }

    // Pass the updated pemesanan data with remaining_time to the view
