...
<body class="font-sans bg-gray-100 text-gray-900">
    ...
    <nav class="flex items-center justify-between bg-white shadow-md px-6 py-4">
        ...
        <div class="flex items-center w-full">
            <form action="{{ route('search') }}" method="GET" class="flex items-center bg-gray-100 rounded-md shadow-inner px-4 py-2 w-full md:w-1/3">
        ...
        <!-- User Profile and Logout (Always visible on Desktop) -->
    ...
    <div id="mobileMenu" class="md:hidden flex-col bg-white shadow-md absolute top-16 right-0 w-full p-4 space-y-4 z-10 hidden">
        <a href="{{ route('dashboard') }}" class="text-gray-700 hover:text-blue-600 py-2 px-4 rounded transition">Dashboard</a>
        <a href="{{ route('pemesanan.index') }}" class="text-gray-700 hover:text-blue-600 py-2 px-4 rounded transition">Pemesanan</a>
        <form action="{{ route('logout') }}" method="POST">
            @csrf
            <button type="submit" class="text-gray-700 hover:text-red-700 py-2 px-4 rounded transition">Logout</button>
        </form>
    </div>
