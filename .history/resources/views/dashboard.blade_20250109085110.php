            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($buses as $bus)
                    <div class="border p-4 rounded shadow">
                        ...
                        @foreach($bus->busKelas as $busKelas)
                            <p><strong>Class:</strong> {{ $busKelas->kelas->nama_kelas }}</p>
                        @endforeach

                        @foreach($bus->busRute as $busRute)
                            @php
                                $harga = $busRute->rute->harga->firstWhere('id_kelas', $bus->busKelas->first()->id_kelas);
                            @endphp
                            @if ($harga)
                        ...
                        <a href="{{ route('pemesanan.create') }}" class="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded">
                            Pesan Sekarang!
                        </a>
                    </div>
                @endforeach
