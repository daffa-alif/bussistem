...
<form action="{{ route('detail_pemesanan.store', $pemesanan->id_pemesanan) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6 w-1/2 mx-auto">
    ...
    <div>
        ...
        <select name="id_kursi_bus" id="id_kursi_bus" required class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            @foreach($kursiBus as $kursi)
                <option value="{{ $kursi->id_kursi }}" data-kelas-id="{{ $kursi->id_kelas }}">
                    {{ $kursi->nomor_kursi }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <p id="total_harga" class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm bg-gray-100 text-gray-600">
            @if ($detailPemesanan)
                Rp {{ number_format($detailPemesanan->total_harga, 0, ',', '.') }}
            @else
    ...
    <div class="flex justify-between">
        <button type="submit" name="action" value="add_another" class="bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-200">
            Add Another
        </button>
        <button type="submit" name="action" value="finish" class="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 transition duration-200">
            Finish
        </button>
    </div>
</form>
   ...
   document.getElementById('id_kursi_bus').addEventListener('change', function () {
    ...
    if (kursiBusId) {
        ...
        fetch(`/get-harga/${idKelas}`)
            ...
            .then(data => {
                if (data.harga_kursi) {
                    // Format the price using the 'id-ID' locale for Indonesian currency
                    const formattedPrice = new Intl.NumberFormat('id-ID', {
                        style: 'currency',
                        currency: 'IDR'
                    }).format(data.harga_kursi);

                    // Update the total harga display element
