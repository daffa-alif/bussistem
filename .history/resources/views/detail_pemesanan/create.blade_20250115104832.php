...
@section('content')
<form action="{{ route('detail_pemesanan.store', $id_pemesanan) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6 w-1/2 mx-auto">
    @csrf

    <div>
        <label for="id_kursi_bus" class="block text-sm font-medium text-gray-700">Kursi Bus</label>
        <input type="text" name="id_kursi_bus" id="id_kursi_bus" value="{{ $kursiBus->nomor_kursi }}" readonly class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
    ...
    <div>
        ...
        <input type="number" step="0.01" name="total_harga" id="total_harga" value="{{ $total_harga }}" required class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" readonly>
        @error('total_harga')
            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
        @enderror
    </div>
