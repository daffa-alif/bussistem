...
<div class="container mx-auto p-6">
    ...
    <form action="{{ route('kursibus.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6 w-1/2 mx-auto">
        @csrf
        ...
        <div class="mb-6">
            ...
            <select name="id_kelas" id="id_kelas" class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                ...
                @foreach($kelas as $kls)
                    <option value="{{ $kls->id_kelas }}" class="text-black-800">{{ $kls->name }}</option>
                @endforeach
            </select>
            @error('id_kelas')
            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
