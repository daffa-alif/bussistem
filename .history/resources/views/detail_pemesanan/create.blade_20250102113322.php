...
<form action="{{ route('detail_pemesanan.store', $pemesanan->id_pemesanan) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6 w-1/2 mx-auto">
    ...
    <div>
        ...
        <input type="number" step="0.01" name="harga_kursi" id="harga_kursi" required class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" readonly>
    </div>

    <div>
        <label for="total_harga" class="block text-sm font-medium text-gray-700">Total Harga</label>
        <input type="number" step="0.01" name="total_harga" id="total_harga" required class="mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    </div>
...
<script>
    document.getElementById('id_kursi_bus').addEventListener('change', function() {
        ...
        if (kursiBusId) {
            fetch(`/get-harga/${kursiBusId}`)
                .then(response => response.json())
                .then(data => {
                    if (data.harga_kursi) {
                        document.getElementById('harga_kursi').value = data.harga_kursi;
                        calculateTotal();
                    }
                })
    ...
    function calculateTotal() {
        var harga = parseFloat(document.getElementById('harga_kursi').value) || 0;
        var totalHarga = harga; // Modify as per your calculation logic
