...
@section('content')
    ...
    <!-- Assuming the URL will now include id_pemesanan in the route -->
    <form action="{{ route('detail_pemesanan.store', $id_pemesanan) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
        @csrf

        <!-- If the user is admin, allow them to select a user -->
    ...
    <script>
        ...
        // If a seat is selected, append it as a hidden input before submitting the form
        kursiBusSelect.addEventListener('change', function() {
            const selectedSeatId = kursiBusSelect.value;
            const hiddenInput = document.createElement('input');
            hiddenInput.type = 'hidden';
            hiddenInput.name = 'id_kursi';  // Make sure this matches the parameter in your store method
            hiddenInput.value = selectedSeatId;
