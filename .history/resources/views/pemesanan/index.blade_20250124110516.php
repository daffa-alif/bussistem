            ...
            @forelse ($pemesanan as $item)
                <div class="border rounded-lg shadow p-4">
                    ...
                    @if ($item->status === 'pending')
                        <div class="mt-2 countdown-container" id="countdown-{{ $item->id_pemesanan }}" data-created-at="{{ $item->created_at }}" data-remaining-time="{{ $item->remaining_time }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            countdownElements.forEach(function(element) {
                const remainingTime = parseInt(element.dataset.remainingTime); // Get remaining time passed from controller
                const countdownTimer = element.querySelector('.countdown-timer');

                let timeLeft = remainingTime * 60; // Convert remaining minutes to seconds
