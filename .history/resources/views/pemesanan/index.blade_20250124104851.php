                ...
                @foreach ($pemesanan as $key => $item)
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            ...
                            @endif
                            <!-- Countdown Timer -->
                            <div id="countdown-{{ $item->id_pemesanan }}" class="mt-2">
                                @if ($item->status === 'pending')
                                    <p>Time remaining: <span class="countdown-timer" data-created-at="{{ $item->tanggal_pemesanan }}"></span> minutes</p>
                                @endif
                            </div>
                        </td>
                    </tr>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const pemesananItems = @json($pemesanan);

            pemesananItems.forEach(item => {
                if (item.status === 'pending') {
                    const createdAt = new Date(item.tanggal_pemesanan);
                    const currentTime = new Date();
                    const diffTime = currentTime - createdAt;
                    const minutesRemaining = Math.max(0, 15 - Math.floor(diffTime / 60000));

                    if (minutesRemaining > 0) {
                        const countdownElement = document.querySelector(`#countdown-${item.id_pemesanan} .countdown-timer`);
                        countdownElement.textContent = minutesRemaining;
                    } else {
                        // Automatically set to cancelled if time is up
                        const cancelForm = document.getElementById('cancel-form-' + item.id_pemesanan);
