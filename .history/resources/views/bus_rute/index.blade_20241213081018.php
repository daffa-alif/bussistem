            ...
            <tbody>
                @foreach($busRutes as $item)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-2">
                            <form action="{{ route('bus_rute.update', ['id_bus_rute' => $busRute->id_bus_rute]) }}" method="POST">
                                @csrf
                                @method('PUT')
                            
                                {{-- Debug the generated route --}}
                                <p>{{ route('bus_rute.update', ['id_bus_rute' => $busRute->id_bus_rute]) }}</p>
                            </form>
                            
                            <form action="{{ route('bus_rute.destroy', $item->id_bus_rute) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
