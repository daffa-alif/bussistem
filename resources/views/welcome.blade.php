...
body {
    ...
    <div class="flex flex-col items-center justify-center min-h-screen text-center text-white">
        <h1 class="text-4xl lg:text-6xl font-extrabold mb-4 text-orange-600 tracking-wide">
    Welcome to JOVA TERMINAL
</h1>
<p class="text-lg lg:text-xl text-gray-200 italic mb-6">
...
<div class="bg-gray-50 py-12">
    <h2 class="text-4xl font-bold text-center text-gray-900 mb-6 animate-fadeIn">
        Our Travels
    </h2>
    <p class="text-center text-lg text-gray-700 mb-10 animate-fadeIn">
        We travel around Indonesia, connecting cities and regions across the country.
    </p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($buses as $bus)
                <div class="bg-white shadow-lg rounded-lg overflow-hidden transform hover:scale-105 transition duration-300">
                    <div class="p-6">
                        <h3 class="text-2xl font-bold text-gray-800">{{ $bus->nama_bus }}</h3>
                        <p class="mt-2 text-gray-600">🚍 Plate: <strong>{{ $bus->plat_nomor }}</strong></p>
                        <p class="mt-1 text-gray-600">🪑 Capacity: <strong>{{ $bus->kapasitas }} seats</strong></p>
                        @foreach($bus->busRute as $busRute)
                            <p class="mt-2 text-gray-600">📍 Route: <strong>{{ $busRute->rute->asal }} → {{ $busRute->rute->tujuan }}</strong></p>
                        @endforeach
                        @if ($bus->busKelas->isNotEmpty())
                            <p class="mt-2 text-gray-600">🎟 Class: <strong>{{ $bus->busKelas->first()->kelas->nama_kelas }}</strong></p>
                        @endif
                    </div>
                    <div class="bg-blue-600 text-white text-center py-3">
                        <a href="{{ route('pemesanan.create', ['bus_id' => $bus->id_bus]) }}" class="block font-semibold hover:bg-blue-700 transition">
                            Book Now!
                        </a>
                    </div>
                </div>
