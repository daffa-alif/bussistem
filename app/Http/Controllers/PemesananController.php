...
public function cancel($id)
{
    ...
    // Update the status of the pemesanan to 'cancelled'
    $pemesanan->status = 'cancelled';
    $pemesanan->save();
    
    // Retrieve all booked seats related to this pemesanan and set them to 'available'
    $detailPemesanan = DetailPemesanan::where('id_pemesanan', $id)->get();
    
    foreach ($detailPemesanan as $detail) {
        $kursiBus = KursiBus::find($detail->id_kursi_bus);
        if ($kursiBus) {
    ...
    return redirect()->back()->with('success', 'Booking has been cancelled and seats have been released.');
}
...
public function cancelTimeout($id)
{
    ...
    // Ensure the booking is still in 'pending' status
    if ($pemesanan->status === 'pending') {
        $pemesanan->status = 'cancelled'; // Change the status to cancelled
        $pemesanan->save(); // Save the change
    }
...
public function update(Request $request, $id)
{
    ...
    // If status is being updated to 'cancelled', release the seats
